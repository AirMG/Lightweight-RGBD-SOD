...
class Eval_thread():
    def __init__(self, loader, method, dataset, output_dir, cuda):
        ...
        if output_dir != "":
            ...
            if not os.path.exists(self.logfile):
                self.LOG("     NJU2K             NLPR             STERE           RGBD135           LFSD             SIP      \n")
    ...
    def run(self):
        ...
        if self.logfile != "":
            self.LOG(
                '  {:.4f} {:.4f}  '.format(s, mae))
        return '[cost:{:.4f}s]{} dataset with {} method get {:.4f} S-measure, {:.4f} mae..'.format(
            time.time() - start_time, self.dataset, self.method, s, mae)
        '''
        self.LOG(
            '{} dataset with {} method get {:.4f} mae, {:.4f} max-fmeasure, {:.4f} max-Emeasure, {:.4f} S-measure..\n'.format(
                self.dataset, self.method, mae, max_f, max_e, s))
