...
class ImageDataTrain(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        depth_quality = [float(self.sal_list[item % self.sal_num].split()[3])]
        ...
        depth_quality = torch.Tensor(depth_quality)


        sample = {'images': sal_image, 'depths': sal_depth, 'gts': sal_label,'depth_quality':depth_quality}
        return sample
...
class ImageDataTest(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        gt = load_sal_label(os.path.join(self.data_root, self.image_list[item].split()[2]))
        image = torch.Tensor(image)
        depth = torch.Tensor(depth)
        return {'image': image, 'name': self.image_list[item % self.image_num].split()[0].split('/')[1], 'size': im_size, 'depth': depth, 'GT':gt}
...
def cv_random_crop(image, depth, label):
    ...
    image = image[:, top: top + 352, left: left + 352]
    depth = depth[:, top: top + 352, left: left + 352]
    label = label[:, top: top + 352, left: left + 352]
